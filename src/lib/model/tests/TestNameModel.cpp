...
static void TestValueInitialization(nlTestSuite *inSuite, void *inContext __attribute__((unused)))
{
    ...
    NameModel                     lNameModel_1;
    NameModel                     lNameModel_2;
    NameModel                     lNameModel_3;
    NameModel                     lNameModel_4;
    NameModel                     lNameModel_5;
    NameModel                     lNameModel_6;
    Status                        lStatus;

    // Test 1: Test invalid names.

    // Test 1.1: Test a null value.

    lStatus = lNameModel_1.Init(lNameConstant_1);
    NL_TEST_ASSERT(inSuite, lStatus == -EINVAL);

    lStatus = lNameModel_2.Init(lNameConstant_1, 0);
    NL_TEST_ASSERT(inSuite, lStatus == -EINVAL);

    // Test 1.2: Test a too-long value with a null-terminated C string.

    lStatus = lNameModel_1.Init(lNameConstant_2);
    NL_TEST_ASSERT(inSuite, lStatus == -ENAMETOOLONG);

    // Test 1.3: Test a too-long value with a pointer and a length.

    lStatus = lNameModel_2.Init(lNameConstant_2, strlen(lNameConstant_2));
    NL_TEST_ASSERT(inSuite, lStatus == -ENAMETOOLONG);

    // Test 1.4: Test a too-long value with a C++ Standard Library string.

    lStatus = lNameModel_1.Init(lNameConstant_3);
    NL_TEST_ASSERT(inSuite, lStatus == -ENAMETOOLONG);

    // Test 2: Test valid names.

    // Test 2.1: Test a null-terminated C string that is under the
    //           maximum length.

    lStatus = lNameModel_1.Init(lNameConstant_4);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    // Test 2.2: Test a pointer and a length that is under the maximum
    //           length.

    lStatus = lNameModel_2.Init(lNameConstant_4, strlen(lNameConstant_4));
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    // Test 2.3: Test a C++ Standard Library string that is under the
    //           maximum length.

    lStatus = lNameModel_3.Init(lNameConstant_5);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    // Test 2.4: Test a null-terminated C string that is exactly the
    //           maximum length.

    lStatus = lNameModel_4.Init(lNameConstant_6);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    // Test 2.5: Test a pointer and a length that is exactly the maximum
    //           length.

    lStatus = lNameModel_5.Init(lNameConstant_6, strlen(lNameConstant_6));
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    // Test 2.6: Test a C++ Standard Library string that is exactly the
    //           maximum length.

    lStatus = lNameModel_6.Init(lNameConstant_7);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
}
...
static void TestObjectInitialization(nlTestSuite *inSuite, void *inContext __attribute__((unused)))
{
    ...
    lStatus = lNameModel_1.Init();
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lStatus = lNameModel_2.Init(lNameModel_1);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
}
...
static void TestObservation(nlTestSuite *inSuite, void *inContext __attribute__((unused)))
{
    ...
    lStatus = lNameModel_1.GetName(lNameValue);
    ...
    lStatus = lNameModel_1.Init(lNameConstant);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    ...
    lStatus = lNameModel_2.GetName(lNameValue);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    NL_TEST_ASSERT(inSuite, strcmp(lNameValue, lNameConstant) == 0);
}
...
static void TestNameMutation(nlTestSuite *inSuite, void *inContext __attribute__((unused)))
{
    ...
    lStatus = lNameModel_2.Init();
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lStatus = lNameModel_3.Init();
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lStatus = lNameModel_4.Init();
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    ...
    lStatus = lNameModel_1.SetName(lNameConstant_1);
    NL_TEST_ASSERT(inSuite, lStatus == -EINVAL);

    lStatus = lNameModel_2.SetName(lNameConstant_1, 0);
    NL_TEST_ASSERT(inSuite, lStatus == -EINVAL);

    // Test 1.1.2: Test a too-long value with a null-terminated C string.

    lStatus = lNameModel_1.SetName(lNameConstant_2);
    NL_TEST_ASSERT(inSuite, lStatus == -ENAMETOOLONG);

    // Test 1.1.3: Test a too-long value with a pointer and a length.

    lStatus = lNameModel_2.SetName(lNameConstant_2, strlen(lNameConstant_2));
    NL_TEST_ASSERT(inSuite, lStatus == -ENAMETOOLONG);
    ...
    lStatus = lNameModel_1.SetName(lNameConstant_3);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    ...
    lStatus = lNameModel_2.SetName(lNameConstant_3, strlen(lNameConstant_3));
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    ...
    lStatus = lNameModel_3.SetName(lNameConstant_4);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lStatus = lNameModel_3.GetName(lNameValue);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    ...
    lStatus = lNameModel_4.SetName(lNameConstant_4, strlen(lNameConstant_4));
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lStatus = lNameModel_4.GetName(lNameValue);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    NL_TEST_ASSERT(inSuite, strcmp(lNameValue, lNameConstant_4) == 0);
    ...
    lStatus = lNameModel_4.SetName(lNameConstant_4);
...
static void TestEquality(nlTestSuite *inSuite, void *inContext __attribute__((unused)))
{
    ...
    lAreEqual = (lNameModel_1 == lNameModel_2);
    NL_TEST_ASSERT(inSuite, lAreEqual == true);

    // Test 2: Test that two value initialized objects are equivalent.

    lStatus = lNameModel_3.Init(lNameConstant_1);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lStatus = lNameModel_4.Init(lNameConstant_1);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lAreEqual = (lNameModel_3 == lNameModel_4);
    NL_TEST_ASSERT(inSuite, lAreEqual == true);

    // Test 3: Test that two object initialized objects are
    //         equivalent.

    lStatus = lNameModel_5.Init(lNameModel_4);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lAreEqual = (lNameModel_5 == lNameModel_4);
    NL_TEST_ASSERT(inSuite, lAreEqual == true);

    // Test 4: Test that an object is equivalent to a NULL-terminated
    //         C string.

    lAreEqual = (lNameModel_3 == lNameConstant_1);
    NL_TEST_ASSERT(inSuite, lAreEqual == true);

    // Test 5: Test that an object is equivalent to a C++ Standard
    //         Library string.

    lAreEqual = (lNameModel_3 == lNameConstant_2);
    NL_TEST_ASSERT(inSuite, lAreEqual == true);

    // Test 6: Test than an unintialized object is not equivalent to
    //         an initialized object, a NULL-terminated C string, or a C++
    //         Standard Library string.

    lAreEqual = (lNameModel_6 == lNameModel_3);
    NL_TEST_ASSERT(inSuite, lAreEqual == false);

    lAreEqual = (lNameModel_6 == lNameConstant_1);
    NL_TEST_ASSERT(inSuite, lAreEqual == false);

    lAreEqual = (lNameModel_6 == lNameConstant_2);
    NL_TEST_ASSERT(inSuite, lAreEqual == false);
}
...
static void TestAssignment(nlTestSuite *inSuite, void *inContext __attribute__((unused)))
{
    ...
    lStatus = lNameModel_3.Init(lNameConstant);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lNameModel_4 = lNameModel_3;
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);
    ...
    lStatus = lNameModel_4.Init(lNameModel_3);
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lNameModel_5 = lNameModel_4;
    NL_TEST_ASSERT(inSuite, lStatus == kStatus_Success);

    lAreEqual = (lNameModel_4 == lNameModel_5);
    NL_TEST_ASSERT(inSuite, lAreEqual == true);
}
